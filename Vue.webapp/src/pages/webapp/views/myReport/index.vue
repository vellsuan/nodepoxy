<template>
  ...
  <div style="height: 100%;width: 100%">
    ...
    <div v-else class="register">
      <div :class="{'reg-top':true}">
        <x-header v-if="hander"
                  :left-options="{showBack: false,backText: '',preventGoBack:true}" :class="{'topmargin':margintop}">
          我的报告
        </x-header>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getvideo(item) {
        ...
        this.api.post(advices, reponse => {
          ...
          if (reponse.list.length > 0) {
            ...
            let workerInfo = list.workerInfo;
            _t.createTimeStr = list.createTimeStr && list.createTimeStr.slice(0, 10) || '';
            _t.workerName = list.workerName || '';
            _t.goodAt = workerInfo.goodAt || '';
            _t.videoUrl = list.videoUrl || '';
            _t.workerTitle = workerInfo.workerTitle || '';
            _t.introduction = workerInfo.introduction || '';
          }
          this.pushvideo(item)
        });
      },
      ...
      usermsg() {
        ...
        _t.api.post(userparams, response => {
          ...
          window.localStorage.email = response.user.email;//用户邮箱保存
    ...
    mounted() {
      setTimeout(function () {
        let dom = document.getElementById('waiindex');
